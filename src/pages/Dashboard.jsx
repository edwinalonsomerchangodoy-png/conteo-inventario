...
export default function Dashboard({ stock, conteos, tiendaActiva, programaActivo }) {
  const universoSet = useMemo(() => {
    if (programaActivo) {
      const codigosBase =
        programaActivo.tipo === 'manual'
          ? programaActivo.codigos || []
          : stock
              .filter((r) => (r[programaActivo.tipo] || '').toString().trim() === programaActivo.valor)
              .map((r) => r.codigo)

      const canonicos = codigosBase.map((cod) => {
  ...
  const porMarca = useMemo(() => agruparPorDimension(conteosValidos, 'marca'), [conteosValidos])
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-soft text-sm mt-1">
          Seguimiento del ciclo {programaActivo ? `"${programaActivo.nombre}"` : 'completo de la tienda'}
          {tiendaActiva && (
            <>
              {' '}
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card className="p-5">
          <p className="text-sm font-medium mb-1">Avance del ciclo</p>
      ...
      <GraficoDimension titulo="Diferencias por marca" datos={porMarca} />
    </div>
  )
}
